...
split <- sample.split(df_cat_clean$Type, SplitRatio = 0.7)
...
ggplotly(ggplot(test_pred$x %>% as.data.frame() %>% cbind(Type=test$Type),
       aes(x=LD2, y=LD3, col=as.factor(Type))) + geom_point(size=3) +
  scale_color_brewer(palette = 'YlGnBu') + labs(col = "Type") + theme_minimal())
...
ggplotly(ggplot(test_pred$x %>% as.data.frame() %>% cbind(Type=test$Type),
       aes(x=LD4, y=LD5, col=as.factor(Type))) + geom_point(size=3) +
  scale_color_brewer(palette = 'YlGnBu') + labs(col = "Type") + theme_minimal())
```
...
```{r}
# train.qda <- train[!(train$Type %in% c(5,6,7)),]
# train.qda$Type <- droplevels(train.qda$Type)

# table(df_cat_clean$Type)
table(df_cat$Type)

df_cat.qda <- df_cat[df_cat$Type != 6,]
df_cat.qda$Type <- droplevels(df_cat.qda$Type)

# mod_qda <- qda(Type~RI+Na+Mg+Al+Si+K+Ca, data = train.qda)
mod_qda <- qda(Type~RI+Na+Mg+Al+Si+K+Ca, data = df_cat.qda)
mod_qda
```
...
```{r}
test_pred.qda <- predict(mod_qda, test)
# test_pred.qda$class == test$Type
```
...
```{r}
# ggplotly(ggplot(test_pred.qda$x %>% as.data.frame() %>% cbind(Type=test$Type),
#        aes(x=LD1, y=LD2, col=as.factor(Type))) + geom_point(size=3) +
#   scale_color_brewer(palette = 'YlGnBu') + labs(col = "Type") + theme_minimal())
# 
# ggplotly(ggplot(test_pred$x %>% as.data.frame() %>% cbind(Type=test$Type),
...
```{r}
# tab <- table(test$Type, test_pred.qda$class)
# tab
```
