...
In order to perform LDA, normality and homodestacity assumptions have to be validated for predictor variables (features) are said to be normally distributed and with equal covariance matrices within each class.
...
The histograms seem to approve normality for all the features except **RI**, **Mg** and **K**. The QQplots shown below also confirm this assumption, although, the presence of outliers is notorious and may have to be checked as commented further on.

_The **Mg** variable presents a lot samples with zero values, which alterates the distribution from normality. There are two brs with a high frequency of samples, which can also be noted from the QQplot, making it differ from the normality line and suggest that maybe this variable should be treated as a categorical variable._
